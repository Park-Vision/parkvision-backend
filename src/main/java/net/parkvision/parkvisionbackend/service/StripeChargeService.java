...
public class StripeChargeService {
    ...
    @Autowired
    public StripeChargeService(StripeChargeRepository stripeChargeRepository, PaymentRepository paymentRepository, ReservationRepository reservationRepository) {
    ...
    public StripeCharge createStripeCharge(StripeCharge stripeCharge){
        ...
        if (!_paymentRepository.existsById(stripeCharge.getPayment().getId())) {
            throw new IllegalArgumentException("Payment with ID " + stripeCharge.getPayment().getId() + " does not exist.");
        }
        if (!_reservationRepository.existsById(stripeCharge.getReservation().getId())) {
            throw new IllegalArgumentException("Reservation with ID " + stripeCharge.getReservation().getId() + " does not exist.");
        }
